...
def write_coeffs_file(path: str, now: datetime, coeffs: np.ndarray) -> None:
    ...
    ออกแบบให้ copy ตัวเลขแต่ละบรรทัดไปวางใน input ของ gold_har_forecast_draft.pine
    ได้ตรงๆ -- b0/b1/b2/b3 เรียงลำดับเดียวกับ input group ในไฟล์ Pine"""
    b0, b1, b2, b3 = coeffs
    with open(path, "w", encoding="utf-8") as f:
        f.write(f"# gold_har_baseline.py -- fit ล่าสุด {now:%Y-%m-%d} (UTC)\n")
        f.write("# copy ตัวเลขด้านล่างไปวางใน input ของ Gold HAR Forecast (Draft).pine ตามลำดับ\n")
        f.write(f"b0={b0:.10g}\n")
        f.write(f"b1={b1:.10g}\n")
        f.write(f"b2={b2:.10g}\n")
        f.write(f"b3={b3:.10g}\n")
